...
type Wsc struct {
	Config                  *WscConfig                  // 配置信息
	WebSocket               *WebSocket                  // 底层WebSocket
	onConnected             func()                      // 连接成功回调
	onConnectError          func(err error)             // 连接异常回调，在准备进行连接的过程中发生异常时触发
	onDisconnected          func(err error)             // 连接断开回调，网络异常，服务端掉线等情况时触发
	onClose                 func(code int, text string) // 连接关闭回调，服务端发起关闭信号或客户端主动关闭时触发
	onSentError             func(err error)             // 发送消息异常回调
	onTextMessageSent       func(message string)        // 发送Text消息成功回调
	onBinaryMessageSent     func(data []byte)           // 发送Binary消息成功回调
	onPingReceived          func(appData string)        // 接受到Ping消息回调
	onPongReceived          func(appData string)        // 接受到Pong消息回调
	onTextMessageReceived   func(message string)        // 接受到Text消息回调
	onBinaryMessageReceived func(data []byte)           // 接受到Binary消息回调
}

type WscConfig struct {
	WriteWait         time.Duration // 写超时
	MaxMessageSize    int64         // 支持接受的消息最大长度，默认512字节
	MinRecTime        time.Duration // 最小重连时间间隔
	MaxRecTime        time.Duration // 最大重连时间间隔
	RecFactor         float64       // 每次重连失败继续重连的时间间隔递增的乘数因子，递增到最大重连时间间隔为止
	MessageBufferSize int           // 消息发送缓冲池大小，默认256
}
...
type WebSocket struct {
	...
	connMu        *sync.RWMutex // 加锁避免重复关闭管道
...
func (wsc *Wsc) SetConfig(config *WscConfig) {
	wsc.Config = config
}
...
func (wsc *Wsc) Connect() {
	...
	//var rand = rand.New(rand.NewSource(time.Now().UnixNano())) //go1.20现在自动为全局随机数生成器Int生成一个随机值
	...
	for {
		...
		// 开启协程读
		go func() {
			defer fmt.Println("websocket: 读协程退出")
			for {
		...
		// 开启协程写
		go func() {
			defer fmt.Println("websocket: 写协程退出")
			for {
				select {
